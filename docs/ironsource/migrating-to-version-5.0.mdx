---
title: Migrating to version 5.0
---
...
The iOS implementation has been updated to v8.6.0 which should bring a range of bug fixes and improvements however the implementation remains unchanged. 
...
You no longer have to manually add all of the ironSource, androidx, play services related manifest entries as these are now added automatically by the gradle implementation.



#### Mediation

If you are using mediation with the IronSource extension you will need to ensure you have updated all the mediation adapters to the latest versions.

You should remove all manifest additions related to the mediation adapters as these are now handled by the gradle implementation. 
You can check the documentation for the individual mediation adapters for more information on the required manifest additions. 

However, the only required addition now is that of the Admob parameters as shown below:
