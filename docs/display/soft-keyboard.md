...
We are still working on improving this functionality.
:::
...
>
> **In order to use this functionality on Android you must use this extension to set a [display mode](display-modes.md). If you don't set a display mode through this extension then the keyboard information may not be correct.**
>
> ```actionscript
...
function keyboard_activateHandler( event:SoftKeyboardEvent ):void
{
    ...
    var visibility:int = Display.instance.getSystemUiVisibility();
    Display.instance.setSystemUiVisibility(
...
function keyboard_deactivateHandler( event:SoftKeyboardEvent ):void
{
    ...
    Display.instance.setDisplayMode( DisplayMode.IMMERSIVE );
}
```
